...
private HashMap<String, String> paramsPermisoEscritura = new HashMap<String, String>();
private HashMap<String, String> paramsPermisoEscrituraVuelta = new HashMap<String, String>();
...
public void guardarSaltos() {
    ...
    for (int i = 0; i < polaca.size(); i++) {
        ...
        if (polaca.get(i).toString().equals("inicio_funcion")) {
            paramsPermisoEscritura.put(polaca.get(i - 2).toString(), "@aux_param" + polaca.get(i - 1).toString());
            paramsPermisoEscrituraVuelta.put("@aux_param" + polaca.get(i - 1).toString(), polaca.get(i - 2).toString());
        }
        if (polaca.get(i).toString().equals("CALL")) {
            paramsPermisoEscritura.put("@aux_param" + polaca.get(i - 1).toString(), polaca.get(i - 2).toString());
            paramsPermisoEscrituraVuelta.put(polaca.get(i - 2).toString(), "@aux_param" + polaca.get(i - 1).toString());
        }
    }
}
...
public void leer() {
    ...
    for (int i = 0; i < polaca.size(); i++) { //ESCRIBO LOS LABEL CUANDO LLEGUE A LA POSICION TAL
        ...
        if (pilaVar.peek().toString().equals("inicio_funcion")) { //ME FIJO SI EMPIEZA UNA FUNCION PORQUE ESCRIBE EN OTRO LADO
            ...
            //paramsPermisoEscritura.put(aux_param_declarada.toString(), "@aux_param" + nom_fun);     //Para llamados a funciones dentro de funciones
            //System.out.println("aca:" + paramsPermisoEscritura);
            funciones.append("@FUNCTION_" + nom_fun + ":" + "\r\n");
            if ((AnalizadorLexico.tablaSimbolos.get(aux_param_declarada.toString()).tipo.equals("uslinteger")))
                funciones.append("MOV EAX, @aux_param" + nom_fun + "\r\n" + "MOV " + aux_param_declarada + ", EAX" + "\r\n"); //asigno el valor del parametro real al de la funcion
            else        //TODO EL IF ANTERIOR Y EL SIGUIENTE SON POSIBLE CAUSA DE PROBLEMAS
                if ((AnalizadorLexico.tablaSimbolos.get(aux_param_declarada.toString()).tipo.equals("single")))
                    funciones.append("FLD " + "@aux_param" + nom_fun + "\r\n" + "FSTP " + aux_param_declarada + "\r\n");
                                ...
                                if ((AnalizadorLexico.tablaSimbolos.get(primerOperando.toString()).tipo.equals("uslinteger")) && ((AnalizadorLexico.tablaSimbolos.get(segundoOperando.toString()).tipo.equals("uslinteger")))) {
                                    ...
                                    if ((AnalizadorLexico.tablaSimbolos.get(primerOperando.toString()).tipo.equals("single")) && ((AnalizadorLexico.tablaSimbolos.get(segundoOperando.toString()).tipo.equals("single")))) {
                                        ...
                                        if ((AnalizadorLexico.tablaSimbolos.get(primerOperando.toString()).tipo.equals("uslinteger")) && ((AnalizadorLexico.tablaSimbolos.get(segundoOperando.toString()).tipo.equals("single")))) {
                                            ...
                                            t.setUso("variable");
											t.tipo="uslinteger";// TODO CREO QUE ESTO NO SIRVE PARA NADA PERO TAMPOCO ROMPE NADA
                                            ...
                                            t.setUso("variable");
											t.tipo="single";// TODO CREO QUE ESTO NO SIRVE PARA NADA PERO TAMPOCO ROMPE NADA
                                            AnalizadorLexico.tablaSimbolos.put(t.lexema, t);
                                        }
                                    }
                                }
...
public void generarCodigoInteger(StringBuilder operador, StringBuilder primerOperando, StringBuilder segundoOperando, StringBuilder escritura) { //para los dos integer
    ...
    switch (op) {
        case "+":// TODO  AVISO Le saque el control de overflow a la suma... testear
            escritura.append("MOV EAX, " + primerOperando + "\r\n" + "ADD EAX, " + segundoOperando + "\r\n" + "MOV " + "@aux" + contador + ", EAX" + "\r\n");
            aux = new StringBuilder("@aux" + contador);
            pilaVar.push(aux);

            break;
        case "-":
            escritura.append("MOV EAX, " + segundoOperando + "\r\n" + "SUB EAX, " + primerOperando + "\r\n" + "JS @LABEL_RESUL_NEG" + "\r\n" + "MOV " + "@aux" + contador + ", EAX" + "\r\n");
            aux = new StringBuilder("@aux" + contador);
            pilaVar.push(aux);
...
public void generarCodigoSingle(StringBuilder operador, StringBuilder primerOperando, StringBuilder segundoOperando, StringBuilder escritura) { //para los dos single
    ...
    StringBuilder aux = null;
    switch (op) {
        case "+": //TODO AVISO TAMBIEN SAQUE EL CHEQUEO DE OVERFLOW ACA.. SOLO CHEQUEAMOS EN PRODUCTO
            escritura.append("FLD " + primerOperando + "\r\n" + "FLD " + segundoOperando + "\r\n" + "FADD" + "\r\n" + "FSTP " + "@aux" + contador + "\r\n");
            aux = new StringBuilder("@aux" + contador);
            pilaVar.push(aux);

            break;
        case "-":
            escritura.append("FLD " + segundoOperando + "\r\n" + "FLD " + primerOperando + "\r\n" + "FSUB" + "\r\n" + "FSTP " + "@aux" + contador + "\r\n");
            aux = new StringBuilder("@aux" + contador);
            pilaVar.push(aux);
...
public void generarDeclaracion() {
    ...
    for (int uwu = 0; uwu < flotantes.size(); uwu++) {
        String declarado = flotantes.get(uwu).toString();
